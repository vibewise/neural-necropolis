...
async function handleRequest(
  ...
  if (method === "GET" && segments.length === 1 && segments[0] === "health") {
    ...
    sendJson(res, 200, {
      ...
      maxActiveJobsGlobal: config.maxActiveJobsGlobal,
      maxActiveJobsPerOwner: config.maxActiveJobsPerOwner,
  ...
  if (
    ...
    if (activeJobs > 0) {
      throw conflict(
        "active_jobs",
        `Cannot purge hosted data while ${activeJobs} hosted job(s) are still active. Cancel or wait for them first.`,
      );
    }

    await clearRunnerData(paths);
    sendJson(res, 200, {
      ok: true,
      cleared: {
        manifests: manifests.length,
...
function buildJobRecord(
  ...
  const heroId = makeUniqueHeroSlug(
    request.hero?.id?.trim() || manifest.agent.displayName,
  );
  return {
...
function makeUniqueHeroSlug(seed: string): string {
  return `${normalizeResourceId(seed)}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
}
