...
- manifest example: [MANIFEST.example.json](MANIFEST.example.json)
- profile example: [PROFILES.example.json](PROFILES.example.json)
...
Use the `local-ollama` profile from [PROFILES.example.json](PROFILES.example.json) and make sure your local Ollama-compatible endpoint is running.
...
```powershell
$env:PROMPT_RUNNER_MODEL_PROFILES_FILE = "docs/prompt-runner/PROFILES.example.json"
```
...
```powershell
Invoke-RestMethod -Method Post -Uri http://127.0.0.1:4010/manifests -ContentType "application/json" -InFile docs/prompt-runner/MANIFEST.example.json
```
