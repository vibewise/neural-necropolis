...
- Move, attack, interact, use_item, rest, wait, and cast_spell are the action kinds.
- cast_spell costs one turn and reveals positions of a category (treasury, monsters, heroes, buildings, prisoner). Results appear in SPELL DISCOVERIES next turn. Re-casting the same spell refreshes the data.
...
function describeSpellDiscoveries(
  discoveries: NonNullable<VisionData["spellDiscoveries"]>,
  currentTurn: number,
  heroPosition: { x: number; y: number },
): string {
  return discoveries
    .map((d) => {
      const age = currentTurn - d.discoveredTurn;
      const freshness = d.mobile
        ? ` (discovered ${age} turn${age === 1 ? "" : "s"} ago, positions may have changed)`
        : ` (discovered ${age} turn${age === 1 ? "" : "s"} ago)`;
      const locs =
        d.positions.length === 0
          ...
          : d.positions
              .map(
                (p) =>
                  `(${p.x},${p.y}) dist ${manhattanDistance(heroPosition, p)}`,
              )
              .join(", ");
      return `- ${d.spell}${freshness}: ${locs}`;
    })
    .join("\n");
}
...
function buildUserPrompt(vision: VisionData): string {
  ...
  if (vision.spellDiscoveries && vision.spellDiscoveries.length > 0) {
    sections.push(
      `### SPELL DISCOVERIES (locations revealed by spells)\n${describeSpellDiscoveries(vision.spellDiscoveries, vision.turn, vision.hero.position)}`,
    );
  }
